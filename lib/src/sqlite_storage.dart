...
class SqlitePersistentStorage implements PersistentStorage {
  ...
  SqlitePersistentStorage(
      ...
      : _migrationOptions = migrationOptions ?? [],
        _persistentStorageMigrator =
            persistentStorageMigrator ?? SqlPersistentStorageMigrator();
  ...
  Future<List<Task>> retrieveAllPausedTasks() async {
    ...
    return result
        .map((e) => Task.createFromJson(jsonDecode(e[objectColumn] as String)))
  ...
  Future<List<ResumeData>> retrieveAllResumeData() async {
    ...
    return result
        .map((e) => ResumeData.fromJson(jsonDecode(e[objectColumn] as String)))
        .toList(growable: false);
  }
  ...
  Future<List<TaskRecord>> retrieveAllTaskRecords() async {
    ...
    return result
        .map((e) => TaskRecord.fromJson(jsonDecode(e[objectColumn] as String)))
        .toList(growable: false);
  }
  ...
  ///
  /// Inserts or updates
  Future<void> store(String table, String taskId, Map<String, dynamic> json) =>
      db.insert(
          table,
          {
            taskIdColumn: taskId,
