Extension package for [background_downloader](https://pub.dev/packages/background_downloader) that adds `SqlitePersistentStorage` to the downloader, backed by SQLite, instead of the `LocalStorePersistentStorage` that is used by the downloader by default. 

To use `SqlitePersistentStorage`, pass an instance to the downloader on the very first call: `FileDownloader(persistentStorage: SqlitePersistentStorage());`


# About SQLite storage

`PersistentStorage` to back the database in the downloader, using
an SQLite database as its own backend. Uses the `sqflite` package, so is limited to platforms supported by that
package.
...
The package also comes with more migration options, including from Flutter Downloader
